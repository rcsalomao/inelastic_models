...
double calc_epsilon_equiv(std::span<double> const epsilon_pos) {
    double epsilon_equiv{1e-14};
    for (auto eps : epsilon_pos) {
        epsilon_equiv += eps * eps;
    }
    return std::sqrt(epsilon_equiv);
}

double calc_alpha_t(std::span<double> const epsilon_t,
                    std::span<double> const epsilon_pos, double epsilon_equiv) {
    double epsilon_equiv_2 = std::pow(epsilon_equiv, 2);
    double alpha_t{0};
...
std::tuple<double, double> calc_D_kappa(double const* sigma, double young,
    ...
    double epsilon_pos[3];
    calc_valores_positivos(epsilon_princ, epsilon_pos);
